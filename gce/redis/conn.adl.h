...
namespace adata
{
  ...
  ADATA_INLINE void read( stream_ty& stream, ::gce::redis::adl::conn& value)
  {
  ...
  template <typename stream_ty>
  ADATA_INLINE void skip_read(stream_ty& stream, ::gce::redis::adl::conn* value)
  {
    (value);
    skip_read_compatible(stream);
  }

  ADATA_INLINE int32_t size_of(const ::gce::redis::adl::conn& value)
  {
    int32_t size = 0;
    uint64_t tag = 1ULL;
    {
  ...
  template<typename stream_ty>
  ADATA_INLINE void write(stream_ty& stream , const ::gce::redis::adl::conn&value)
  {
    uint64_t tag = 1ULL;
    write(stream,tag);
    if(stream.error()){return;}
