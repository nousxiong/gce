...
public:
...
#endif
  {
    ...
    try
    {
      ...
      {
        strand_t& snd = strand_list_.make_back(boost::ref(*ios_));
        threaded_service_list_.make_back(boost::ref(*this), boost::ref(snd), index);
        stackful_service_list_.make_back(boost::ref(*this), boost::ref(snd), index);
        stackless_service_list_.make_back(boost::ref(*this), boost::ref(snd), index);
#ifdef GCE_LUA
        lua_service_list_.make_back(boost::ref(*this), boost::ref(snd), index);
#endif
        socket_service_list_.make_back(boost::ref(*this), boost::ref(snd), index);
        acceptor_service_list_.make_back(boost::ref(*this), boost::ref(snd), index);
      }

      for (std::size_t i=0; i<attrs_.nonblocked_num_; ++i, ++index)
      {
        strand_t& snd = strand_list_.make_back(boost::ref(*ios_));
        nonblocked_service_list_.make_back(boost::ref(*this), boost::ref(snd), index);
        nonblocked_actor_list_.make_back(boost::ref(*this), boost::ref(nonblocked_service_list_[i]), index);
      }
