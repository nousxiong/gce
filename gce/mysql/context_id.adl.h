...
namespace adata
{
  ...
  ADATA_INLINE void read( stream_ty& stream, ::gce::mysql::adl::context_id& value)
  {
  ...
  template <typename stream_ty>
  ADATA_INLINE void skip_read(stream_ty& stream, ::gce::mysql::adl::context_id* value)
  {
    (value);
    skip_read_compatible(stream);
  }

  ADATA_INLINE int32_t size_of(const ::gce::mysql::adl::context_id& value)
  {
    int32_t size = 0;
    uint64_t tag = 1ULL;
    {
  ...
  template<typename stream_ty>
  ADATA_INLINE void write(stream_ty& stream , const ::gce::mysql::adl::context_id&value)
  {
    uint64_t tag = 1ULL;
    write(stream,tag);
    if(stream.error()){return;}
