...
public:
  static void run()
  {
    std::cout << "ssl_session_ut begin." << std::endl;
    ...
    std::cout << "ssl_session_ut end." << std::endl;
  }
...
private:
  ...
  static void echo_client(stackful_actor self)
  {
    ...
    try
    {
      ...
      ssl_skt->set_verify_callback(boost::bind(&ssl_session_ut::verify_certificate, _arg1, _arg2, lg));
  ...
  static void echo_server(stackful_actor self, std::string const& port)
  {
    ...
    try
    {
      ...
      ssl_ctx.set_password_callback(boost::bind(&ssl_session_ut::get_password));
      ...
      while (true)
      {
        ...
        if (!ec)
        {
          aid_t cln = spawn(self, boost::bind(&ssl_session_ut::echo_session<Parser>, _arg1), monitored);
  ...
  static void test_base()
  {
    ...
    try
    {
      ...
      aid_t len_svr = spawn(
        base_svr, 
        boost::bind(&ssl_session_ut::echo_server<parser::length>, _arg1, "23333"), 
        monitored
        );
      ...
      aid_t reg_svr = spawn(
        base_svr, 
        boost::bind(&ssl_session_ut::echo_server<parser::regex>, _arg1, "23334"), 
        monitored
        );
      ...
      for (size_t i=0; i<cln_count; ++i)
      {
        if (i % 2 == 0)
        {
          aid_t cln = spawn(
            base_cln, 
            boost::bind(&ssl_session_ut::echo_client<parser::length>, _arg1), 
            monitored
            );
        ...
        else
        {
          aid_t cln = spawn(
            base_cln, 
            boost::bind(&ssl_session_ut::echo_client<parser::regex>, _arg1), 
            monitored
            );
