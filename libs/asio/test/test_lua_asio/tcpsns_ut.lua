...
gce.actor(
  function ()
    ...
    local len_svr = gce.spawn('test_lua_asio/tcpsns_echo_server.lua', gce.monitored)
    gce.send(len_svr, 'init', asio.plength, len_port)
    ec = gce.match('ready').guard(len_svr).recv()
    assert(ec == gce.ec_ok, ec)

    -- spawn pregex server
    local reg_svr = gce.spawn('test_lua_asio/tcpsns_echo_server.lua', gce.monitored)
    ...
    -- spawn clients
    local cln = nil 
    cln = gce.spawn('test_lua_asio/tcpsns_echo_client.lua', gce.monitored)
