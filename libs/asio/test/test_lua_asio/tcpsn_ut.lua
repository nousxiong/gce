...
gce.actor(
  function ()
    ...
    local len_svr = gce.spawn('test_lua_asio/tcpsn_echo_server.lua', gce.monitored)
    gce.send(len_svr, 'init', asio.plength, len_port)
    ec = gce.match('ready').guard(len_svr).recv()
    assert(ec == gce.ec_ok, ec)

    -- spawn pregex server
    local reg_svr = gce.spawn('test_lua_asio/tcpsn_echo_server.lua', gce.monitored)
    ...
    for i=1, cln_count do
      if i % 2 == 0 then 
        local cln = gce.spawn('test_lua_asio/tcpsn_echo_client.lua', gce.monitored)
