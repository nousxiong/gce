...
public:
  static void run()
  {
    std::cout << "tcp_sessions_ut begin." << std::endl;
    ...
    std::cout << "tcp_sessions_ut end." << std::endl;
  }
...
private:
  ...
  static void echo_client(stackful_actor self)
  {
    ...
    try
    {
      ...
      typedef session<Parser, tcp_socket_t, stackful_actor> session_t;
      typedef std::pair<boost::shared_ptr<session_t>, size_t> session_data;
      typedef std::map<snid_t, session_data> session_list_t;
      ...
      for (size_t i=0; i<cln_count; ++i)
      {
        ...
        session_list.insert(std::make_pair(opt.id, std::make_pair(sn, ecount)));
        sn->open();
      }
      ...
      while (true)
      {
        ...
        match_t type;
        snid_t snid;
        self->match(sn_open, sn_recv, sn_close, type).raw(msg).recv(snid);
        if (type == sn_close)
        {
          msg >> ec;
          session_list.erase(snid);
  ...
  static void echo_server(stackful_actor self, std::string const& port)
  {
    ...
    try
    {
      ...
      while (true)
      {
        ...
        if (!ec)
        {
          aid_t cln = spawn(self, boost::bind(&tcp_sessions_ut::echo_session<Parser>, _arg1), monitored);
  ...
  static void test_base()
  {
    ...
    try
    {
      ...
      aid_t len_svr = spawn(
        base_svr, 
        boost::bind(&tcp_sessions_ut::echo_server<parser::length>, _arg1, "23333"), 
        monitored
        );
      ...
      aid_t reg_svr = spawn(
        base_svr, 
        boost::bind(&tcp_sessions_ut::echo_server<parser::regex>, _arg1, "23334"), 
        monitored
        );
...

      {
        aid_t cln = spawn(
          base_cln, 
          boost::bind(&tcp_sessions_ut::echo_client<parser::length>, _arg1), 
          monitored
          );
      ...
      }
      {
        aid_t cln = spawn(
          base_cln, 
          boost::bind(&tcp_sessions_ut::echo_client<parser::regex>, _arg1), 
          monitored
          );
