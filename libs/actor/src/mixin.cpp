...
mixin::mixin(context& ctx, std::size_t id, attributes const& attrs)
  ...
  , mtx_(GCE_CACHE_ALIGNED_NEW(boost::shared_mutex), detail::cache_aligned_deleter())
  , cv_(GCE_CACHE_ALIGNED_NEW(boost::condition_variable_any), detail::cache_aligned_deleter())
  , cache_pool_list_(GCE_CACHE_ALIGNED_NEW(cache_pool_list_t), detail::cache_aligned_deleter())
{
  ...
  try
  {
    ...
    for (std::size_t i=0; i<attrs.per_mixin_cache_; ++i, ++index)
    {
      ...
      cac_pool = cache_pool_ptr(GCE_CACHE_ALIGNED_NEW(detail::cache_pool)(ctx, index, attrs, true));
    }

    owner_ = select_cache_pool();
    slice_pool_.reset(
      GCE_CACHE_ALIGNED_NEW(detail::slice_pool_t)(
        owner_.get(), this,
