...
namespace adata
{
  ...
  ADATA_INLINE void read( stream_ty& stream, ::cluster5::endpoint_list& value)
  {
  ...
  template <typename stream_ty>
  ADATA_INLINE void skip_read(stream_ty& stream, ::cluster5::endpoint_list* value)
  {
    (value);
    skip_read_compatible(stream);
  }

  ADATA_INLINE int32_t size_of(const ::cluster5::endpoint_list& value)
  {
    int32_t size = 0;
    uint64_t tag = 0ULL;
  ...
  template<typename stream_ty>
  ADATA_INLINE void write(stream_ty& stream , const ::cluster5::endpoint_list&value)
  {
    uint64_t tag = 0ULL;
    if(!value.list_.empty()){tag|=1ULL;}
    write(stream,tag);
