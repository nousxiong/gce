...
namespace cluster2 {
  ...
  {
    ::std::vector< ::cluster2::node_info > list_;
    node_info_list()
    {}
  };
...
namespace adata
{
  ...
  ADATA_INLINE void read( stream_ty& stream, ::cluster2::node_info& value)
  {
  ...
  template <typename stream_ty>
  ADATA_INLINE void skip_read(stream_ty& stream, ::cluster2::node_info* value)
  {
    (value);
    skip_read_compatible(stream);
  }

  ADATA_INLINE int32_t size_of(const ::cluster2::node_info& value)
  {
    int32_t size = 0;
    uint64_t tag = 1ULL;
  ...
  template<typename stream_ty>
  ADATA_INLINE void write(stream_ty& stream , const ::cluster2::node_info&value)
  {
    uint64_t tag = 1ULL;
    if(!value.ep_.empty()){tag|=2ULL;}
    write(stream,tag);
  ...
  template<typename stream_ty>
  ADATA_INLINE void read( stream_ty& stream, ::cluster2::node_info_list& value)
  {
    ::std::size_t offset = stream.read_length();
    uint64_t tag = 0;
    read(stream,tag);
  ...
  template <typename stream_ty>
  ADATA_INLINE void skip_read(stream_ty& stream, ::cluster2::node_info_list* value)
  {
    (value);
    skip_read_compatible(stream);
  }

  ADATA_INLINE int32_t size_of(const ::cluster2::node_info_list& value)
  {
    int32_t size = 0;
    uint64_t tag = 0ULL;
    ...
    if(tag&1ULL)
    {
      ...
      size += size_of(len);
      for (::std::vector< ::cluster2::node_info >::const_iterator i = value.list_.begin() ; i != value.list_.end() ; ++i)
      {
        size += size_of(*i);
      }
    }
  ...
  template<typename stream_ty>
  ADATA_INLINE void write(stream_ty& stream , const ::cluster2::node_info_list&value)
  {
    uint64_t tag = 0ULL;
    if(!value.list_.empty()){tag|=1ULL;}
    write(stream,tag);
    ...
    if(tag&1ULL)
    {
      ...
      write(stream,len);
      int32_t count = 0;
      for (::std::vector< ::cluster2::node_info >::const_iterator i = value.list_.begin() ; i != value.list_.end() ; ++i, ++count)
      {
        {write(stream,*i);}
